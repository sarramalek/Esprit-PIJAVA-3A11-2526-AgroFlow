...
public class ServiceExamen implements IService<examens> {
    ...
    @Override
    public List<examens> recuperer() throws SQLException {
        List<examens> liste = new ArrayList<>();
        String sql = "SELECT * FROM examens_sante";
        Statement st = cnx.createStatement();
        ResultSet rs = st.executeQuery(sql);
        while (rs.next()) {
            liste.add(new examens(
                    rs.getInt("id"),
                    rs.getDate("date_examen"),
    ...
    @Override
    public examens rechercherParId(int id) throws SQLException {
        String sql = "SELECT * FROM examens_sante WHERE id=?";
        PreparedStatement ps = cnx.prepareStatement(sql);
        ps.setInt(1, id);
