...
// iText 7 — dépendances : com.itextpdf:kernel, com.itextpdf:layout, com.itextpdf:io (v7.2.5)
...
// Apache POI — Export Excel (com.itextpdf n'est pas utilisé pour Excel)
...
/**
 * Controller complet — Gestion des Achats
 *
 * Fonctionnalités :
 *  • Tableau : Date / Quantité / Machine (nom seul) / Client / CIN
 *  • Recherche temps réel
 *  • Effacer recherche
 *  • Filtre par Machine
 *  • Tri : Plus récent | Plus ancien | Qté ↑ | Qté ↓ | Machine A→Z
 *  • Réinitialiser tout
 *  • Export PDF (iText 7)
 *  • Export Excel (Apache POI)
 *  • Statistiques Bar Chart (Canvas JavaFX)
 *  • Boutons CRUD en bas : Ajouter / Modifier / Supprimer
 *  • Dialog Ajouter/Modifier : machine affichée par nom uniquement (sans ID)
 *  • Navigation sidebar
 */
public class AfficherAchatsController implements Initializable {
    ...
    // Ordre de tri courant (appliqué lors des filtrages)
    ...
    public static class AchatVM {
        ...
        private final String machineNom; // Marque + Modèle uniquement
    ...
    // ── MachineItem helper (affiche nom sans ID dans le combo) ────
    private static class MachineItem {
        final int    idM;
        final String label; // "Marque Modele"
    ...
    private void afficherDialog(AchatVM existant) {
        ...
        // Machine — ComboBox affichant "Marque Modele" uniquement (sans ID)
        ...
        btnOK.setStyle("-fx-background-color: #27ae60; -fx-text-fill: white; -fx-font-size: 14px; -fx-font-weight: bold; -fx-cursor: hand; -fx-background-radius: 6;");

        Button btnAnn = new Button("❌ Annuler");
        btnAnn.setPrefSize(150, 40);
        btnAnn.setStyle("-fx-background-color: #95a5a6; -fx-text-fill: white; -fx-font-size: 14px; -fx-font-weight: bold; -fx-cursor: hand; -fx-background-radius: 6;");
        ...
        btnOK.setOnAction(e -> {
            ...
            try {
                ...
                achat.setIdM(machineCombo.getValue().idM);   // on garde idM en interne
        ...
        btns.getChildren().addAll(btnOK, btnAnn);
    ...
    private boolean valider(DatePicker dp, TextField qte,
        ...
        if (dp.getValue() == null)                   { showWarn("Champ requis",  "Sélectionnez une date."); return false; }
        if (dp.getValue().isAfter(LocalDate.now()))  { showWarn("Date invalide", "La date ne peut pas être dans le futur."); return false; }
        if (dp.getValue().isBefore(LocalDate.of(2000,1,1))) { showWarn("Date invalide", "Date trop ancienne (avant 2000)."); return false; }
        if (qte.getText().trim().isEmpty())           { showWarn("Champ requis",  "Entrez une quantité."); return false; }
        if (!qte.getText().trim().matches("\\d+"))   { showWarn("Format invalide","La quantité doit être un entier positif."); return false; }
        int q = Integer.parseInt(qte.getText().trim());
        if (q < 1 || q > 10000)                      { showWarn("Valeur invalide","Quantité entre 1 et 10 000."); return false; }
        if (machine.getValue() == null)               { showWarn("Champ requis",  "Sélectionnez une machine."); return false; }
        if (client.getValue() == null)                { showWarn("Champ requis",  "Sélectionnez un client."); return false; }
        return true;
    }
