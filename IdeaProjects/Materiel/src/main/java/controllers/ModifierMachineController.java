...
public class ModifierMachineController {
    ...
    // ══════════════════════════════════════════════════════════════════════════
    //  PASSER LA MACHINE À MODIFIER (appelé depuis AfficherMachinesController)
    // ══════════════════════════════════════════════════════════════════════════

    /**
     * Reçoit la machine sélectionnée et pré-remplit le formulaire.
     * Doit être appelé APRÈS le chargement du FXML (initialize déjà exécuté).
     */
    ...
    /** Pré-remplit tous les champs avec les valeurs actuelles de la machine. */
    private void remplirChamps() {
        if (machineAModifier == null) return;
        ...
        String etatActuel = machineAModifier.getEtatM();
        if (etatActuel != null && ETATS_VALIDES.contains(etatActuel)) {
            tfEtatM.setValue(etatActuel);
        } else if (etatActuel != null) {
            // Valeur inconnue : on l'ajoute temporairement et on la sélectionne
            tfEtatM.getItems().add(etatActuel);
            tfEtatM.setValue(etatActuel);
        }
    ...
    private void handleModifier() {
        if (machineAModifier == null) {
            alerte("Erreur", "Aucune machine à modifier.", Alert.AlertType.ERROR);
            return;
        }
        ...
        try {
            // Unicité numéro de série (sauf si inchangé)
            if (!numeroSerie.equalsIgnoreCase(machineAModifier.getNumeroSerie())
                    && numeroSerieExiste(numeroSerie)) {
            ...
            alerte("Succès", "Machine modifiée avec succès !", Alert.AlertType.INFORMATION);
            retourListeMachines();

        } catch (Exception e) {
            e.printStackTrace();
            alerte("Erreur", "Erreur lors de la modification : " + e.getMessage(), Alert.AlertType.ERROR);
        }
    }
    ...
    /** État : obligatoire, sélectionné dans le ComboBox. */
